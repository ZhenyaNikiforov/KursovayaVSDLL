    ...
    while (true) { //-- Основной цикл программы;
        ...
        while (true) { //- общий цикл проведения гонки;
            ...
            if (startRace) {//-если флаг старта гонки startRace поднят
                ...
                /*Сортируем массивы транс. ср-в и рез-тов, соответст. им: */
                bool continuation = true;//Переменная продолжение цикла сортировки
                while (continuation) {//-продолжаем цикл, пока переменная продолжения сортировки истинно
                    bool permutation = false;//переменная перестановок изначально false
                    for (int i = 0; i < results.size(); ++i) {//проходим массив рез-тов;
                        if ((i + 1) < results.size()) {//если значен. на 1 больше тек. инд. меньше длины массива...
                            if (results[i] > results[i + 1]) {//если текущ. значен. массива больше след. значен...
                                std::swap(results[i], results[i + 1]);//меняем их местами...
                                std::swap(ourVehicles[i], ourVehicles[i + 1]);//также меняем местами трансп. ср-ва...
                                permutation = true;//...и поднимаем флаг совершившейся перестановки;
                            };//дно вопроса если младший результат больше;
                        };//дно вопроса существует ли старший индекс;
                    };//дно цикла пробежки по массиву;
                    if (!permutation) { continuation = false; };//если перестановки не было, спускаем флаг продолжения;
                };//дно цикла пузырьковой сортировки по возрастанию;
                cout << "Результаты гонки:\n";//выводим результаты гонки
                for (int i = 0; i < ourVehicles.size(); ++i) {//перебираем сортированный массив избранн. транс. ср-в... 
                    cout << i + 1 << ". " << vehicles[ourVehicles[i] - 1] << ": " << results[i] << endl;//номер, назв-е, рез-т
                };//дно цикла отбражения;
